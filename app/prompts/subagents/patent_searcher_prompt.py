...
1. 查看'技术交底书结构化信息文档'，根据提供的技术关键词，使用MCP工具搜索相似专利
   ...
   - 优先搜索中国专利（CN）
   - 搜索'GRANT'状态的授权专利
   - 返回前10个最相关结果时，调用'patent_search'工具使用 `top_k=10`，不要使用 `num`
   - `sort` 可选值为 `relevance` / `new` / `old`；默认相关度可传 `sort="relevance"` 或省略，程序会归一化为不向 SerpAPI 发送 `sort`
...
6. 同时写入结构化检索结果 `02_research/patent_search_results.json`；`02_research/prior_art_analysis.md` 是唯一现有技术分析产物
