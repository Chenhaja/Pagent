...
def test_skill_loader_reads_markdown_skill_docs(tmp_path) -> None:
    """skill_loader 应从 Markdown 技能文档目录读取白名单文档。"""
    skill_dir = tmp_path / "skills_docs"
    skill_dir.mkdir()
    (skill_dir / "patent_drafting.md").write_text("# 专利写作技能", encoding="utf-8")
    tool = SkillLoaderTool(Settings(skill_dir=str(skill_dir)))

    observation = tool.run({"skill_name": "patent_drafting"})

    assert observation.error is None
    assert observation.evidence[0]["skill_name"] == "patent_drafting"
    assert observation.evidence[0]["content"] == "# 专利写作技能"
    assert observation.evidence[0]["path"].endswith("patent_drafting.md")
...
def test_skill_loader_rejects_unknown_and_path_traversal(tmp_path) -> None:
    ...
    unknown = tool.run({"skill_name": "patent_qa"})
    escaped = tool.run({"skill_name": "../patent_drafting"})
...
def test_default_tool_registry_describes_markdown_skill_loader(tmp_path) -> None:
    ...
    assert spec.input_schema["properties"]["skill_name"]["enum"] == ["patent_drafting", "mermaid"]
